...
def main():
	simple_dispatch = simple_dispatch_similarity_score()
	filter_by_simple_dispatch_similarity_score(simple_dispatch, 0.02)


def calc_rmse():
	mef_results = pd.read_csv(MEF_FORECASTS, index_col = 1, parse_dates = [1]).dropna()
	simple_dispatch_rmse = _rmse(mef_results['simple dispatch'], mef_results['baseline'])
	print('RMSE (Simple Dispatch): ', simple_dispatch_rmse)

	simple_dispatch_rmse = _rmse(mef_results['neural net'], mef_results['baseline'])
	print('RMSE (Neural Net): ', simple_dispatch_rmse)
...
def filter_by_simple_dispatch_similarity_score(df, percentile):
	...
	df.to_csv('simple_dispatch_high_similarity_score' + percentile + '.csv')
